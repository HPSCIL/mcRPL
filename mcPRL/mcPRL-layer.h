#ifndef mcPRL_LAYER_H
#define mcPRL_LAYER_H
...
namespace mcPRL {
  class Layer {
    public:
      ...
      const mcPRL::CellspaceGeoinfo * glbGeoinfo() const;
      ...
      void initCellspaceInfo(const mcPRL::SpaceDims &dims,
                             const char *aTypeName,
                             size_t typeSize,
                             const mcPRL::CellspaceGeoinfo *pGeoinfo = NULL,
                             long  tileSize = TILEWIDTH);
      void initCellspaceInfo(const mcPRL::CellspaceInfo &cellspcInfo);
      
	    bool initCellspaceInfoByGDAL();
	    bool initCellspaceInfoByPGTIOL();

      void delCellspaceInfo();
      mcPRL::CellspaceInfo* cellspaceInfo();
      const mcPRL::CellspaceInfo* cellspaceInfo() const;
      ...
      void delCellspace();
      mcPRL::Cellspace* cellspace();
      const mcPRL::Cellspace* cellspace() const;

      /* SubCellspaceInfo */
      int nSubCellspaceInfos() const;
      const list<mcPRL::SubCellspaceInfo>* allSubCellspaceInfos() const;
      bool delSubCellspaceInfo_glbID(int glbID);
      bool delSubCellspaceInfo_lclID(int lclID);
      void clearSubCellspaceInfos();
      const mcPRL::IntVect allSubCellspaceIDs() const;
      int maxSubCellspaceID() const;
      bool calcAllBRs(bool onlyUpdtCtrCell,
                      const mcPRL::Neighborhood *pNbrhd);
      
      mcPRL::SubCellspaceInfo* subCellspaceInfo_glbID(int glbID,
                                                     bool warning = false);
      const mcPRL::SubCellspaceInfo* subCellspaceInfo_glbID(int glbID,
                                                           bool warning = false) const;
      mcPRL::SubCellspaceInfo* subCellspaceInfo_lclID(int lclID,
                                                     bool warning = false);
      const mcPRL::SubCellspaceInfo* subCellspaceInfo_lclID(int lclID,
                                                           bool warning = false) const;
      
      /* SubCellspace */
      int nSubCellspaces() const;
      mcPRL::SubCellspace* addSubCellspace(int glbID,
                                          double *pInitVal = NULL);
      bool delSubCellspace_glbID(int glbID);
      bool delSubCellspace_lclID(int lclID);
      void clearSubCellspaces();
      
      mcPRL::SubCellspace* subCellspace_glbID(int glbID,
                                             bool warning = false);
      const mcPRL::SubCellspace* subCellspace_glbID(int glbID,
                                                   bool warning = false) const;
      mcPRL::SubCellspace* subCellspace_lclID(int lclID,
                                             bool warning = false);
      const mcPRL::SubCellspace* subCellspace_lclID(int lclID,
                                                   bool warning = false) const;
    ...
    private:
      ...
      mcPRL::CellspaceInfo *_pCellspcInfo;
      mcPRL::Cellspace *_pCellspc;

      list<mcPRL::SubCellspaceInfo> _lSubInfos;
      list<mcPRL::CellspaceInfo> _lCellspcInfos;
      list<mcPRL::SubCellspace> _lSubCellspcs;
