...
DeviceProcessor mcPRL::Process::
getDeive()
{
   return _device;
}
const MPI_Comm& mcPRL::Process::
comm() const {
  return _comm;
}
...
const mcPRL::IntVect mcPRL::Process::
allPrcIDs(bool incldMaster,
          bool incldWriter) const {
  mcPRL::IntVect vPrcIDs;
...
bool mcPRL::Process::initCuda()
{
	...
	if(this->isMaster())
	{
		map<string,mcPRL::IntVect>mstrdevcount;
		map<string,mcPRL::IntVect>mstr;
		map<string,mcPRL::IntVect>::iterator itmstr;
		map<string,mcPRL::IntVect>::iterator itdevcount;
		mstr.insert(pair<string,mcPRL::IntVect>(_prcrName,mcPRL::IntVect()));
		mstr.begin()->second.push_back(0);
		mstrdevcount.insert(pair<string,mcPRL::IntVect>(_prcrName,_device.allDevice()));
		for(int i=1;i<_nTotalPrcs;i++)
		{
			...
			mcPRL::IntVect vavalible_recv(devcount_recv);
			...
			if(itmstr==mstr.end())
			{
				mcPRL::IntVect vnpocesses;
				vnpocesses.push_back(i);
				mstr.insert(pair<string,mcPRL::IntVect>(strname, vnpocesses));
				vnpocesses.clear();
				mstrdevcount.insert(pair<string,mcPRL::IntVect>(strname,vavalible_recv));
			}
			else
			{
		...
		for(itmstr=mstr.begin();itmstr!=mstr.end();itmstr++)
		{
			...
			mcPRL::IntVect vnPoID=itmstr->second;
			mcPRL::IntVect vnPoID2=itdevcount->second;
			int nPo=vnPoID.size();
			for(int i=0;i<nPo;i++)
			{
				cout<<vnPoID[i]<<" ";
