...
_checkGDALSettings(GDALDataset *pDataset,
                   int iBand,
                   const mcPRL::CellCoord *pGDALOff,
...
bool mcPRL::Cellspace::
init(mcPRL::CellspaceInfo *pInfo,
     bool ifInitMem) {
  _pInfo = pInfo;
  if(ifInitMem) {
    if(!initMem()) {
...
bool mcPRL::Cellspace::
brupdateCell(const mcPRL::CoordBR &br,void *output)
{
	 bool done = false;
  if(_pInfo->isDataType<bool>()) {
...
mcPRL::Cellspace& mcPRL::Cellspace::
operator=(const mcPRL::Cellspace& rhs) {
  if(this != &rhs) {
    clear();
    _pInfo = rhs._pInfo;
...
const mcPRL::CellspaceInfo* mcPRL::Cellspace::
info() const {
  return _pInfo;
}

mcPRL::CellspaceInfo* mcPRL::Cellspace::
info() {
  return _pInfo;
}

void mcPRL::Cellspace::
info(mcPRL::CellspaceInfo *pInfo) {
  _pInfo = pInfo;
}
...
const mcPRL::LongVect& mcPRL::Cellspace::
...
createGdalDS(GDALDataset *&pDataset,
  ...
  mcPRL::cppType2gdalType(gdalDataType, string(_pInfo->dataType()));
...
bool mcPRL::Cellspace::
creatRandData(const mcPRL::CellCoord *pGDALOff,
             bool warning)
{
	 long iGDALRowOff = 0, iGDALColOff = 0;
	 if(pGDALOff != NULL) {
