...
bool mcPRL::SmplDcmp::
_checkDcmpPrmtrs(list<mcPRL::SubCellspaceInfo> &lSubspcInfos,
  ...
  if(pNbrhd != NULL) {
    ...
    else if(pNbrhd->edgeOption() == mcPRL::FORBID_VIRTUAL_EDGES &&
...
bool mcPRL::SmplDcmp::
rowDcmp(list<mcPRL::SubCellspaceInfo> &lSubspcInfos,
        const mcPRL::SpaceDims &glbDims,
        int nSubspcs,
        const mcPRL::Neighborhood *pNbrhd) const {
  ...
  mcPRL::DomDcmpType dcmpType = mcPRL::ROW_DCMP;
  ...
  for(int iSubspc = 0; iSubspc < nSubspcs; iSubspc++) {
    ...
    if(pNbrhd != NULL) {
      ...
      if(pNbrhd->edgeOption() == mcPRL::NODATA_VIRTUAL_EDGES ||
         pNbrhd->edgeOption() == mcPRL::CUSTOM_VIRTUAL_EDGES) {
        if(iSubspc == 0) {
    ...
    mcPRL::CoordBR subMBR(nwCorner, seCorner);
    mcPRL::SpaceDims subDims(subMBR.nRows(), subMBR.nCols());

    mcPRL::CoordBR subWorkBR;
    if(pNbrhd != NULL) {
    ...
    vector<mcPRL::IntVect> mNbrSpcIDs(2);
    if(iSubspc != 0) {
      mNbrSpcIDs[mcPRL::UPPER_DIR].push_back(iSubspc - 1);
    }
    if(iSubspc != nSubspcs-1) {
      mNbrSpcIDs[mcPRL::LOWER_DIR].push_back(iSubspc + 1);
    }

    lSubspcInfos.push_back(mcPRL::SubCellspaceInfo(iSubspc, dcmpType, glbDims,
...
bool mcPRL::SmplDcmp::
colDcmp(list<mcPRL::SubCellspaceInfo> &lSubspcInfos,
        const mcPRL::SpaceDims &glbDims,
        int nSubspcs,
        const mcPRL::Neighborhood *pNbrhd) const {
  if(!_checkDcmpPrmtrs(lSubspcInfos, glbDims,
  ...
  mcPRL::DomDcmpType dcmpType = mcPRL::COL_DCMP;
  ...
  for(int iSubspc = 0; iSubspc < nSubspcs; iSubspc++) {
    ...
    vector<mcPRL::IntVect> mNbrSpcIDs(2);
    if(iSubspc != 0) {
      mNbrSpcIDs[mcPRL::LEFT_DIR].push_back(iSubspc - 1);
    }
    if(iSubspc != nSubspcs-1) {
      mNbrSpcIDs[mcPRL::RIGHT_DIR].push_back(iSubspc + 1);
    }
...
bool mcPRL::SmplDcmp::
blkDcmp(list<mcPRL::SubCellspaceInfo> &lSubspcInfos,
        const mcPRL::SpaceDims &glbDims,
        int nRowSubspcs,
        int nColSubspcs,
        const mcPRL::Neighborhood *pNbrhd) const {
  ...
  mcPRL::DomDcmpType dcmpType = mcPRL::BLK_DCMP;
  ...
  for(int iRowSubspc = 0; iRowSubspc < nRowSubspcs; iRowSubspc++) {
    ...
    for(int iColSubspc = 0; iColSubspc < nColSubspcs; iColSubspc++) {
      ...
      vector<mcPRL::IntVect> mNbrSpcIDs(8);
