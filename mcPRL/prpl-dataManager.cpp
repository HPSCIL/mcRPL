    ...
    if(readySubspcID != pRPL::ERROR_ID) {
      ...
      switch(evalType) {
        case EVAL_NONE:
          ...
          break;
        case EVAL_ALL:
          if(_evalAll(trans, isGPUCompute,pf,readySubspcID, pRPL::EVAL_WORKBR, ifInitNoData) == pRPL::EVAL_FAILED) {
            _prc.abort();
            return pRPL::EVAL_FAILED;
          }
          break;
      ...
      if(!trans.needExchange() ||
        ...
        switch(evalType) {
          case EVAL_NONE:
            ...
            break;
          case EVAL_ALL:
			  if(_evalAll(trans,isGPUCompute, pf,subspcGlbID, pRPL::EVAL_WORKBR, ifInitNoData) == pRPL::EVAL_FAILED) {
              _prc.abort();
              return pRPL::EVAL_FAILED;
            }
            break;
        ...
        switch(evalType) {
          case EVAL_NONE:
            ...
            break;
          case EVAL_ALL:
            if(_evalAll(trans, isGPUCompute,pf, subspcGlbID, pRPL::EVAL_EDGES, ifInitNoData) == pRPL::EVAL_FAILED) {
              _prc.abort();
              return pRPL::EVAL_FAILED;
            }
            break;
        ...
        for(int iSubspc = 0; iSubspc < pvSubspcIDs->size(); iSubspc++) {
          ...
          switch(evalType) {
            case EVAL_NONE:
              break;
            case EVAL_ALL:
              if(_evalAll(trans, isGPUCompute,pf, subspcGlbID, pRPL::EVAL_INTERIOR, ifInitNoData) == pRPL::EVAL_FAILED) {
                _prc.abort();
                return pRPL::EVAL_FAILED;
              }
              break;
...
pRPL::EvaluateReturn pRPL::DataManager::
_evalAll(pRPL::Transition &trans,bool isGPUCompute,
           pCuf pf,
         int subspcGlbID,
...
finishReadingLayers(pRPL::ReadingOption readOpt) {
  ...
  while(!_vSendSpcReqs.empty() && nCmpltSends != MPI_UNDEFINED) {
    nCmpltSends = _iTransfSpaceTest(vCmpltSendIDs, pRPL::SEND_TRANSF, delSentSpc, false, false);                                   //´«Êý¾Ý
  }
  while(!_vRecvSpcReqs.empty() && nCmpltRecvs != MPI_UNDEFINED) {
