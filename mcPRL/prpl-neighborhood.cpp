...
void pRPL::Neighborhood::addnbrForGpu()
{
	 long long  nbrsize=this->size();
	 vector<pRPL::WeightedCellCoord>coords =this->getInnerNbr(); 
...
isEquallyWeighted(double &weight) const {
 ...
 vector<double> pRPL::Neighborhood::cuWeights()
 {
	 return cuNbrWeights;
 }
  vector<int>pRPL::Neighborhood::cuCoords()
  {
	  return cuNbrCoords;
  }
