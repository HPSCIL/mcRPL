...
mcPRL::CellspaceInfo::
CellspaceInfo(const mcPRL::CellspaceInfo &rhs)
  :_dTypeName("UNKNOWN"),
   _dTypeSize(0),
   _pNoDataVal(NULL),
   _pGeoInfo(NULL),
...
mcPRL::CellspaceInfo::
CellspaceInfo(const mcPRL::SpaceDims &dims,
...
mcPRL::CellspaceInfo& mcPRL::CellspaceInfo::
operator=(const mcPRL::CellspaceInfo &rhs) {
  if(this != &rhs) {
    if(!init(rhs._dims, rhs._dTypeName.c_str(), rhs._dTypeSize, rhs._pGeoInfo, rhs._tileSize)) {
      exit(-1);
    }
...
bool mcPRL::CellspaceInfo::
init(const mcPRL::SpaceDims &dims,
     const char *aTypeName,
     size_t typeSize,
     const mcPRL::CellspaceGeoinfo *pGeoinfo,
     long tileSize) {
  ...
  _dTypeSize = typeSize; 
  if(pGeoinfo != NULL) {
    _pGeoInfo = new mcPRL::CellspaceGeoinfo(*pGeoinfo);
  }
...
initByGDAL(GDALDataset *pDataset,
  ...
  if(!init(mcPRL::SpaceDims(nRows, nCols), dataTypeName.c_str(), dataTypeSize)) {
    return false;
  }
  ...
  mcPRL::CellspaceGeoinfo geoInfo;
...
void mcPRL::CellspaceInfo::
clear() {
  _dims = mcPRL::SpaceDims();
...
initFromBuf(vector<char> &buf) {
  ...
  if(ifGeoref == 'y') {
    ...
    _pGeoInfo = new mcPRL::CellspaceGeoinfo(geoInfo);
  }
  else if(ifGeoref != 'n') {
    cerr << __FILE__ << " function:" << __FUNCTION__ \
...
void mcPRL::CellspaceInfo::
georeference(const mcPRL::CellspaceGeoinfo& geoinfo) {
  if(_pGeoInfo != NULL) {
    delete _pGeoInfo;
    _pGeoInfo = NULL;
  }
...
const mcPRL::CellspaceGeoinfo* mcPRL::CellspaceInfo::
georeference() const {
  return _pGeoInfo;
}
...
const mcPRL::SpaceDims& mcPRL::CellspaceInfo::
dims() const {
  return _dims;
}

long mcPRL::CellspaceInfo::
nRows() const {
  return _dims.nRows();
}
...
const char* mcPRL::CellspaceInfo::
dataType() const {
  return _dTypeName.c_str();
}

size_t mcPRL::CellspaceInfo::
dataSize() const {
  return _dTypeSize;
}
...
mcPRL::CellCoord mcPRL::CellspaceInfo::
...
bool mcPRL::CellspaceInfo::
validBR(const mcPRL::CoordBR& rectangle,
        bool warning) const {
  bool valid = true;
...
mcPRL::GeoCoord mcPRL::CellspaceInfo::
cellCoord2geoCoord(const mcPRL::CellCoord &cCoord) const {
  mcPRL::GeoCoord gCoord;
  if(!isEmpty(true) &&
...
bool mcPRL::CellspaceInfo::
calcWorkBR(mcPRL::CoordBR *pWorkBR,
...
bool mcPRL::CellspaceInfo::
validNbrhd(const mcPRL::Neighborhood &nbrhd,
  ...
  if(nbrhd.edgeOption() == mcPRL::FORBID_VIRTUAL_EDGES) {
...
validNbrhd(const mcPRL::Neighborhood &nbrhd,
           ...
           bool warning) const {
  return validNbrhd(nbrhd, mcPRL::CellCoord(ctrRow, ctrCol), warning);
}
...
validNbrhd(const mcPRL::Neighborhood &nbrhd,
           long idx,
           bool warning) const {
  return validNbrhd(nbrhd, mcPRL::CellCoord(idx, _dims), warning);
}
