...
namespace pRPL {
  class Cellspace {
    public:
	  ...
	  template<class InputType>
	 bool brupdateCellAs(const pRPL::CoordBR &br,void *output);
	 template<class InputType>
	 ...
	 bool createrandForCells(long ncols,long nrows);
      ...
      bool brupdateCell(const pRPL::CoordBR &br,void *output);
      template<class ElemType>
      ...
      */
      ...
      bool readGDALData(GDALDataset *pDataset,
                        ...
                        bool warning = true);
	  bool creatRandData( const pRPL::CellCoord *pGDALOff = NULL,
                        bool warning = true);
...
template<class InputType>
bool pRPL::Cellspace::
brupdateCellAs(const pRPL::CoordBR &br,void *output)
{
	 bool done = false;
	 for(long iRow = br.minIRow(); iRow <= br.maxIRow(); iRow++) 
	  {
		  for(long iCol = br.minICol(); iCol <= br.maxICol(); iCol++) 
		  {
			  ...
			  return done;
				 ...
				 done=updateCellAs<InputType>(pRPL::CellCoord(iRow, iCol),((InputType*)output)[iRow*_pInfo->nCols()+iCol], true);                         //”–Œ Ã‚
				 if(!done)
					 return done;
		  }
	  }
