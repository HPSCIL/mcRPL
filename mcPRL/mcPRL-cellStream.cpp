...
vector<pair<unsigned int, unsigned int> >::const_iterator mcPRL::CellStream::
...
mcPRL::CellStream& mcPRL::CellStream::
operator=(const mcPRL::CellStream &rhs) {
  if(this != &rhs) {
    clear();
    _size = rhs._size;
...
size_t mcPRL::CellStream::
size() const {
  return _size;
}

int mcPRL::CellStream::
...
vector<int>& mcPRL::CellStream::
getCellCounts() {
  return _vCellCounts;
}

const vector<pair<unsigned int, unsigned int> >& mcPRL::CellStream::
getLayerInfos() const {
  return _vLayerInfos;
}

bool mcPRL::CellStream::
resize() {
  if(_aStream != NULL) {
    free(_aStream);
    _aStream = NULL;
...
const vector<int>& mcPRL::CellStream::
getCellCounts() const {
  return _vCellCounts;
}
