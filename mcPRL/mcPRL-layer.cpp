...
const mcPRL::SpaceDims* mcPRL::Layer::
glbDims() const {
...
long mcPRL::Layer::tileSize() const{
  if(_pCellspcInfo == NULL) {
    return 0;
  }
...
const mcPRL::CellspaceGeoinfo* mcPRL::Layer::
glbGeoinfo() const {
  if(_pCellspcInfo == NULL) {
    return NULL;
  }
...
openGdalDS(const char *aFileName,
  ...
  //string filepath="D:\\cuda\\shikong\\testdata\\nd_dem.tif";
  //_pGdalDS = (GDALDataset *)GDALOpen(filepath.c_str(), ioOption);
  //if(_pGdalDS == NULL) {
  //  cerr << __FILE__ << " " << __FUNCTION__ \
  //       << " Error: failed to open GDAL file (" \
  //       << aFileName << ")" << endl;
...
createGdalDS(const char *aFileName,
  ...
  mcPRL::cppType2gdalType(gdalDataType, string(_pCellspcInfo->dataType()));
...
decompose(int nRowSubspcs,
  ...
  mcPRL::DomDcmpMethod dcmpMethod = mcPRL::SNGL_PRC;
  if(nRowSubspcs == 1 && nColSubspcs == 1) {
    dcmpMethod = mcPRL::SNGL_PRC;
  }
  else if(nRowSubspcs > 1 && nColSubspcs == 1) {
    dcmpMethod = mcPRL::SMPL_ROW;
  }
  else if(nRowSubspcs == 1 && nColSubspcs > 1) {
    dcmpMethod = mcPRL::SMPL_COL;
  }
  else if(nRowSubspcs > 1 && nColSubspcs > 1) {
    dcmpMethod = mcPRL::SMPL_BLK;
  }
  else {
    cerr << __FILE__ << " " << __FUNCTION__ \
...
copyDcmp(const Layer *pFromLyr) {
  ...
  for(int iSubInfo = 0; iSubInfo < pFromLyr->nSubCellspaceInfos(); iSubInfo++) {
    _lSubInfos.push_back(mcPRL::SubCellspaceInfo(*(pFromLyr->subCellspaceInfo_lclID(iSubInfo))));
  }
  return true;
}
...
initCellspaceInfo() {
  delCellspaceInfo();
  delCellspace();
  _pCellspcInfo = new mcPRL::CellspaceInfo();
}

void mcPRL::Layer::
initCellspaceInfo(const mcPRL::SpaceDims &dims,
                  const char *aTypeName,
                  size_t typeSize,
                  const mcPRL::CellspaceGeoinfo *pGeoinfo,
                  long tileSize) {
  delCellspaceInfo();
  delCellspace();
  _pCellspcInfo = new mcPRL::CellspaceInfo(dims, aTypeName, typeSize, pGeoinfo, tileSize);
}

void mcPRL::Layer::
initCellspaceInfo(const mcPRL::CellspaceInfo &cellspcInfo) {
  delCellspaceInfo();
  delCellspace();
  _pCellspcInfo = new mcPRL::CellspaceInfo(cellspcInfo);
}
...
mcPRL::CellspaceInfo* mcPRL::Layer::
cellspaceInfo() {
  return _pCellspcInfo;
}

const mcPRL::CellspaceInfo* mcPRL::Layer::
cellspaceInfo() const {
  return _pCellspcInfo;
}
...
createCellspace(double *pInitVal) {
  ...
  _pCellspc = new mcPRL::Cellspace(_pCellspcInfo, true);
...
/*---------------------PGTIOL----------------------------*/
bool mcPRL::Layer::loadCellspaceByPGTIOL() {
...
bool mcPRL::Layer::writeCellspaceByPGTIOL() {
  if(_pCellspcInfo == NULL ||
	 _pCellspc == NULL) {
	cerr << __FILE__ << " " << __FUNCTION__ \
		 << " Error: NULL CellspaceInfo or NULL Cellspace, unable to write to a GDAL dataset" \
...
mcPRL::Cellspace* mcPRL::Layer::
cellspace() {
  return _pCellspc;
}

const mcPRL::Cellspace* mcPRL::Layer::
cellspace() const {
  return _pCellspc;
}
...
const list<mcPRL::SubCellspaceInfo>* mcPRL::Layer::
...
delSubCellspaceInfo_glbID(int glbID) {
  bool found = false;
  list<mcPRL::SubCellspaceInfo>::iterator itrSubInfo = _lSubInfos.begin();
...
mcPRL::SubCellspaceInfo* mcPRL::Layer::
subCellspaceInfo_glbID(int glbID,
                       bool warning) {
  mcPRL::SubCellspaceInfo *pSubInfo = NULL;
  list<mcPRL::SubCellspaceInfo>::iterator itrSubInfo = _lSubInfos.begin();
  while(itrSubInfo != _lSubInfos.end()) {
    if(glbID == itrSubInfo->id()) {
      pSubInfo = &(*itrSubInfo);
...
const mcPRL::SubCellspaceInfo* mcPRL::Layer::
subCellspaceInfo_glbID(int glbID,
                       bool warning) const {
  const mcPRL::SubCellspaceInfo *pSubInfo = NULL;
  list<mcPRL::SubCellspaceInfo>::const_iterator itrSubInfo = _lSubInfos.begin();
  while(itrSubInfo != _lSubInfos.end()) {
    if(glbID == itrSubInfo->id()) {
      pSubInfo = &(*itrSubInfo);
      break;
...
const mcPRL::IntVect mcPRL::Layer::
allSubCellspaceIDs() const {
  mcPRL::IntVect vSubspcIDs;
  list<mcPRL::SubCellspaceInfo>::const_iterator itrSubInfo = _lSubInfos.begin();
  while(itrSubInfo != _lSubInfos.end()) {
...
mcPRL::SubCellspace* mcPRL::Layer::
addSubCellspace(int glbID,
                double *pInitVal) {
  mcPRL::SubCellspace *pSubspc = subCellspace_glbID(glbID);
  if(pSubspc != NULL) {
    cerr << __FILE__ << " " << __FUNCTION__ \
    ...
    mcPRL::SubCellspaceInfo* pSubspcInfo = subCellspaceInfo_glbID(glbID);
    if(pSubspcInfo == NULL) {
      cerr << __FILE__ << " " << __FUNCTION__ \
      ...
      _lCellspcInfos.push_back(mcPRL::CellspaceInfo(pSubspcInfo->dims(),dataType(),dataSize()));
      mcPRL::CellspaceInfo *pCellspcInfo_sub = &(_lCellspcInfos.back());

      double noDataVal = _pCellspcInfo->getNoDataValAs<double>();
      pCellspcInfo_sub->setNoDataValAs<double>(noDataVal);

      if(_pCellspcInfo->isGeoreferenced(false)) {
        mcPRL::CellspaceGeoinfo subspcGeoinfo = _pCellspcInfo->georeference()->subSpaceGeoinfo(*pSubspcInfo);
        pCellspcInfo_sub->georeference(subspcGeoinfo);
      }

      pCellspcInfo_sub->setTileSize(tileSize());

      _lSubCellspcs.push_back(mcPRL::SubCellspace(pCellspcInfo_sub, pSubspcInfo));
...
delSubCellspace_glbID(int glbID) {
  bool found = false;
  list<mcPRL::SubCellspace>::iterator itrSubspc = _lSubCellspcs.begin();
  list<mcPRL::CellspaceInfo>::iterator itrCellspcInfo = _lCellspcInfos.begin();
...
subCellspace_glbID(int glbID,
                   bool warning) {
  mcPRL::SubCellspace *pSubspc = NULL;
  list<mcPRL::SubCellspace>::iterator itrSubspc = _lSubCellspcs.begin();
  while(itrSubspc != _lSubCellspcs.end()) {
    if(glbID == itrSubspc->subInfo()->id()) {
      pSubspc = &(*itrSubspc);
...
const mcPRL::SubCellspace* mcPRL::Layer::
subCellspace_glbID(int glbID,
                   bool warning) const {
  const mcPRL::SubCellspace *pSubspc = NULL;
  list<mcPRL::SubCellspace>::const_iterator itrSubspc = _lSubCellspcs.begin();
  while(itrSubspc != _lSubCellspcs.end()) {
    if(glbID == itrSubspc->subInfo()->id()) {
      pSubspc = &(*itrSubspc);
      break;
...
allUpdatedIdxs(vector<long> &vUpdtGlbIdxs) const {
  ...
  if(_lSubCellspcs.size() > 0) {
    list<mcPRL::SubCellspace>::const_iterator itrSubspc = _lSubCellspcs.begin();
    while(itrSubspc != _lSubCellspcs.end()) {
      const mcPRL::LongVect &vLclIdxs = itrSubspc->getUpdatedIdxs();
...
bool mcPRL::Layer::
loadSubCellspaceByRAND(int glbID)
{
	mcPRL::SubCellspace *pSubspc = subCellspace_glbID(glbID, false);
  if(pSubspc == NULL) {
  ...
  return pSubspc->creatRandData( &gdalOff, true);
}
bool mcPRL::Layer::
...
writeSubCellspaceByGDAL(int glbID) {
  mcPRL::SubCellspace *pSubspc = subCellspace_glbID(glbID, true);
  if(pSubspc == NULL) {
    return false;
  }

  const mcPRL::CellCoord gdalOff = pSubspc->subInfo()->MBR().nwCorner();
  const mcPRL::CoordBR workBR = pSubspc->subInfo()->workBR();
