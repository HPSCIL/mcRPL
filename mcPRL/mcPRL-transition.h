#ifndef mcPRL_TRANSITION_H
#define mcPRL_TRANSITION_H
...
namespace mcPRL {
  ...
  class Transition {
    public:
	  ...
	  bool initGlobalCoords(const vector<mcPRL::CellCoord> &vGlobalCoords);
	  bool initGlobalCoords(const vector<int> &vGlobalCoords);
	   void clearGlobalCoords();
      mcPRL::Cellspace* getCellspaceByLyrName(const string &lyrName);
      const mcPRL::Cellspace* getCellspaceByLyrName(const string &lyrName) const;
      ...
      mcPRL::Neighborhood* getNbrhd();
      const mcPRL::Neighborhood* getNbrhd() const;
  ...
  public:
	   ...
	   mcPRL::EvaluateReturn cuFocalOperator(const mcPRL::CoordBR &br);
	   template<class OperType>
	   mcPRL::EvaluateReturn cuFocalMutiOperator(const mcPRL::CoordBR &br);
	   template<class OperType>
	   mcPRL::EvaluateReturn cuLocalOperator(const mcPRL::CoordBR &br);
	   template<class OperType>
	   mcPRL::EvaluateReturn cuZonelOperator(const mcPRL::CoordBR &br);
	    template<class OperType>
	   mcPRL::EvaluateReturn cuGlobalOperator(const mcPRL::CoordBR &br);
  public:
      mcPRL::EvaluateReturn evalBR(const mcPRL::CoordBR &br,bool isGPUCompute=NULL, mcPRL::pCuf pf=NULL);
      mcPRL::EvaluateReturn evalRandomly(const mcPRL::CoordBR &br);
      mcPRL::EvaluateReturn evalSelected(const mcPRL::CoordBR &br,
       ...
       * before running.
       * */
      virtual bool afterSetCellspaces(int subCellspcGlbIdx = mcPRL::ERROR_ID);
      ...
      virtual mcPRL::EvaluateReturn evaluate(const mcPRL::CellCoord &coord);

      /* Calculate the workload */
      virtual double workload(const mcPRL::CoordBR &workBR) const;

    protected:
      string _primeLyrName;
      vector<string> _vInLyrNames;
      vector<string> _vOutLyrNames;
      map<string, mcPRL::Cellspace*> _mpCellspcs;
	  int _smCount;
      string _nbrhdName;
      mcPRL::Neighborhood *_pNbrhd;
 ...
 template<class T>
 void mcPRL::Transition::addparam(vector<T>vPara)
 {
 ...
 template<class OperType>
  mcPRL::EvaluateReturn mcPRL::Transition::cuFocalMutiOperator(const mcPRL::CoordBR &br)
  {
	//  vector<mcPRL::WeightedCellCoord>coords =_pNbrhd->getInnerNbr(); 
	  ...
	  mcPRL::Cellspace *pRrmSpc=getCellspaceByLyrName(getPrimeLyrName());
	  ...
	  for(int i=0;i<numOutRaster;i++)
	  {
		...
		//  cout<<"成功"<<endl;
		 ...
		 // {
			...
			//	//  cout<<(DataOutType*)outdata[i])[iRow*nwidth+iCol]<<" ";
			//	  pRrmSpc->updateCellAs<DataOutType>(mcPRL::CellCoord(iRow, iCol),((DataOutType*)outdata[i])[iRow*nwidth+iCol], true);
			//  }
		 // }
	  }
	  for(int i=0;i<numOutRaster;i++)
  ...
  template<class OperType>
  mcPRL::EvaluateReturn mcPRL::Transition::cuGlobalOperator(const mcPRL::CoordBR &br)
  {
	  int brminIRow=br.minIRow();
	  int brmaxIRow=br.maxIRow();
	  int brminICol=br.minICol();
  ...
  //2018/10/18
  template<class OperType>
  mcPRL::EvaluateReturn mcPRL::Transition::cuZonelOperator(const mcPRL::CoordBR &br)
  {
	 int brminIRow=br.minIRow();
	  int brmaxIRow=br.maxIRow();
	  int brminICol=br.minICol();
...
//	  cudaFree(d_nbrcoord);
  ...
  template<class OperType>
  mcPRL::EvaluateReturn mcPRL::Transition::cuLocalOperator(const mcPRL::CoordBR &br)
  {
	 // mcPRL::Cellspace *pRrmSpc=getCellspaceByLyrName(getInLyrNames()[0]);
	 // DataInType *d_input;
	 ...
	 // dim3 block(16,16);
  //   dim3 grid(nwidth% 16 == 0 ? nwidth /16 : nwidth / 16 + 1, nheight % 16 == 0 ? nheight /16 : nheight /16 + 1);
	 //// dim3 grid = mcPRL::CuEnvControl::getGrid(nwidth, nheight);
	 ...
	 // {
		...
		//		  pRrmSpc->updateCellAs<DataOutType>(mcPRL::CellCoord(iRow, iCol),output[iRow*nwidth+iCol+nwidth*nheight*i], true);
		//	  }
		//  }
	 // }
...
//	  cudaFree(d_nbrcoord);
  ...
  template<class DataInType, class DataOutType, class OperType>
  mcPRL::EvaluateReturn mcPRL::Transition::cuFocalOperator(const mcPRL::CoordBR &br)
  {
...
//	  cuNbrWeights.push_back(1);
	  ...
	  mcPRL::Cellspace *pIptSpc = getCellspaceByLyrName(getInLyrNames()[0]);
	  ...
	  mcPRL::checkCudaErrors(cudaMalloc(&d_input, sizeof(DataInType)*width*height));
	 mcPRL:: checkCudaErrors(cudaMalloc(&d_output, sizeof(DataOutType)*width*height));
	  mcPRL:: checkCudaErrors(cudaMalloc(&d_nbrcoord, sizeof(int)*nbrsize * 2));
	 mcPRL:: checkCudaErrors(cudaMalloc(&d_weights, sizeof(double)*nbrsize));

	  mcPRL::checkCudaErrors( cudaMemcpy(d_input, input, sizeof(DataInType)*width*height, cudaMemcpyHostToDevice));
	  mcPRL::checkCudaErrors(cudaMemcpy(d_nbrcoord,cucoords, sizeof(int)*nbrsize * 2, cudaMemcpyHostToDevice));
	  mcPRL::checkCudaErrors( cudaMemcpy(d_weights, weights, sizeof(double)*nbrsize, cudaMemcpyHostToDevice));
	  mcPRL::Cellspace *_pSlpCellspc = getCellspaceByLyrName(_vOutLyrNames[0]);
	  ...
	  //dim3 block =mcPRL::CuEnvControl::getBlock2D();
	  //dim3 grid = mcPRL::CuEnvControl::getGrid(width, height);
	  ...
	  mcPRL:: checkCudaErrors(cudaMemcpy(output, d_output, sizeof(DataOutType)*width*height, cudaMemcpyDeviceToHost));
	  ...
	  for(long iRow = br.minIRow(); iRow <= br.maxIRow(); iRow++) 
	  {
		  for(long iCol = br.minICol(); iCol <= br.maxICol(); iCol++) 
		  {
			  ...
			  _pSlpCellspc->updateCellAs<DataOutType>(mcPRL::CellCoord(iRow, iCol),output[iRow*width+iCol], true);
		  }
	  }
	  delete []output;
	  cudaFree(d_input);
