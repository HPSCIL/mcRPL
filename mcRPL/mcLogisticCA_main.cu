...
int main(int argc, char *argv[]) {
 ...
 // InitCUDA(caDM.mpiPrc().id());
  ...
  mcRPL::ReadingOption readOpt = parallelIO ? mcRPL::PARA_READING : mcRPL::CENTDEL_READING;
  mcRPL::WritingOption writeOpt;
  if(opOption != 0) {
    writeOpt = parallelIO ? mcRPL::PARA_WRITING : mcRPL::CENT_WRITING;
  }
  else {
  ...
  /* ----------------------- Initialize Layers ----------------------- */
  mcRPL::Layer *pUrban0 = caDM.addLayerByGDAL("URBAN0", urbanFile.c_str(), 1, true);
  const mcRPL::SpaceDims &glbDims = *(pUrban0->glbDims());
  const mcRPL::CellspaceGeoinfo *pGlbGeoinfo = pUrban0->glbGeoinfo();

  mcRPL::Layer *pLandUse = caDM.addLayerByGDAL(_aInputNames[0], landuseFile.c_str(), 1, true);
  mcRPL::Layer *pElev = caDM.addLayerByGDAL(_aInputNames[1], elevFile.c_str(), 1, true);
  mcRPL::Layer *pSlope = caDM.addLayerByGDAL(_aInputNames[2], slopeFile.c_str(), 1, true);
  mcRPL::Layer *pDist2CityCtr = caDM.addLayerByGDAL(_aInputNames[3], dist2CityCtrFile.c_str(), 1, true);
  mcRPL::Layer *pDist2Transp = caDM.addLayerByGDAL(_aInputNames[4], dist2TranspFile.c_str(), 1, true);
  mcRPL::Layer *pExcluded = caDM.addLayerByGDAL("EXCLUDED", excludedFile.c_str(), 1, true);
  //mcRPL::Layer *pGlbProb2 = caDM.addLayerByGDAL("GLBPROB2",  glb2.c_str(), 1, true);   //201811 15£»
  mcRPL::Layer *pGlbProb = caDM.addLayer("GLBPROB");
  pGlbProb->initCellspaceInfo(glbDims, typeid(float).name(), sizeof(float), pGlbGeoinfo);
  mcRPL::Layer *pRandNumber=caDM.addLayer("RAND");
  mcRPL::Layer *pJointProb = caDM.addLayer("JOINTPROB");
 ...
 //     }
  /* compute global probability */
    mcRPL::pCuf pf;
   pf=&mcRPL::Transition::cuFocalMutiOperator<cuglbProb>;
  cout << caDM.mpiPrc().id() << ": computing global probilities... " << endl;
  mcRPL::Transition  glbProbTrans;
 ...
 //  double timeInitTasking;
  if(taskFarming) {
    ...
    if(!caDM.initTaskFarm(glbProbTrans, mcRPL::CYLC_MAP, nSubspcs2Map, readOpt)) {
      return -1;
    }

    // Task farming
    cout << caDM.mpiPrc().id() << ": task farming...." << endl;
    if(caDM.evaluate_TF(mcRPL::EVAL_ALL, glbProbTrans, readOpt, mcRPL::NO_WRITING, false) != mcRPL::EVAL_SUCCEEDED) {
      return -1;
    }
  }
  else {
    cout << caDM.mpiPrc().id() << ": initializing static tasking...." << endl;
    if(!caDM.initStaticTask(glbProbTrans, mcRPL::CYLC_MAP, readOpt)) {
      return -1;
    }
 // caDM.mpiPrc().sync();
 // if(caDM.mpiPrc().isMaster()) {
 ...
 // }
	...
	if(caDM.evaluate_ST(mcRPL::EVAL_ALL, glbProbTrans, mcRPL::NO_WRITING, true,pf,false) != mcRPL::EVAL_SUCCEEDED) {
      return -1;
    }
  }
  //caDM.closeDatasets();
 ...
 //  caDM.closeDatasets();
  ...
  /* ----------------------- GROWTH ----------------------- */
 mcRPL::Transition jointProbTrans;
  jointProbTrans.setNbrhdByName(pNbrhd3x3->name());

  mcRPL::Transition distDecProbTrans;
 ...
 mcRPL::Transition consProbTrans;
...
//    #endif
	...
	if(!caDM.createLayerGDAL(pJointProb->name(),jointoutfilename.c_str(), "GTiff", NULL)) {
	 ...
	 pf=&mcRPL::Transition::cuFocalMutiOperator<jointProb>;
	 if(caDM.evaluate_ST(mcRPL::EVAL_ALL, jointProbTrans, mcRPL::NO_WRITING,true, pf,false) != mcRPL::EVAL_SUCCEEDED) {
      return -1;
...
//	 	 caDM.closeDatasets();
  ...
  if(caDM.mpiPrc().isMaster()) {
   ...
   /*string distoutfilename;
	...
	if(!caDM.createLayerGDAL(pDistDecayProb->name(),distoutfilename.c_str(), "GTiff", NULL)) {
	 ...
	 pf=&mcRPL::Transition::cuFocalMutiOperator<DistDecayProb>;
	 if(caDM.evaluate_ST(mcRPL::EVAL_ALL, distDecProbTrans, mcRPL::NO_WRITING, true,pf,false) != mcRPL::EVAL_SUCCEEDED) {
      return -1;
    }
	//  caDM.closeDatasets();
  ...
  if(caDM.mpiPrc().isMaster()) {
   ...
   // timeStart = MPI_Wtime();
    ...
    mcRPL::WritingOption currentWriteOpt = mcRPL::NO_WRITING;
	...
	pf=&mcRPL::Transition::cuFocalMutiOperator<constarined>;
	//#ifdef DEBUG
	if(caDM.evaluate_ST(mcRPL::EVAL_ALL, consProbTrans,currentWriteOpt, true,pf,false) != mcRPL::EVAL_SUCCEEDED) {
      return -1;
    }
