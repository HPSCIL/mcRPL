...
template<class Oper>
__global__ void G_GlobalMutiOperator (void **d_pDataIn,int *d_pDataInType,void **d_pDataOut,int *d_pDataOutType,int brminIRow,int brmaxIRow,int brminICol,int brmaxICol,int width, int height, double cellWidth,double cellHeight,double *para,int *GlobalCoords, Oper op)
{/*
	int x_idx = blockIdx.x*blockDim.x + threadIdx.x;
	if (x_idx >brmaxICol||x_idx<brminICol)
		return;
	...
	for(long long i=x_idx;i<width*height;i=i+blockDim.x*gridDim.x)
	{
	    ...
	    if (row>brmaxIRow||row<brminIRow)
		continue;
	op(d_pDataIn,d_pDataInType,d_pDataOut,d_pDataOutType,i, rasterinfo,para,GlobalCoords);
	}
}
template<class Oper>
...
{
public:
	__device__ void operator()(void **focalIn,int *DataInType,void **focalOut,int *DataOutType,int nIndex,rasterInfo rasterinfo,double *para,int *GlobalCoords)
	{
	//	short *pB=(short*)focalIn[0]+nIndex;
		int width=rasterinfo.width;
		int height=rasterinfo.height;
