...
const mcRPL::SpaceDims* mcRPL::Layer::
glbDims() const {
...
long mcRPL::Layer::tileSize() const{
  if(_pCellspcInfo == NULL) {
    return 0;
  }
...
const mcRPL::CellspaceGeoinfo* mcRPL::Layer::
glbGeoinfo() const {
  if(_pCellspcInfo == NULL) {
    return NULL;
  }
...
createGdalDS(const char *aFileName,
  ...
  mcRPL::cppType2gdalType(gdalDataType, string(_pCellspcInfo->dataType()));
...
decompose(int nRowSubspcs,
  ...
  mcRPL::DomDcmpMethod dcmpMethod = mcRPL::SNGL_PRC;
  if(nRowSubspcs == 1 && nColSubspcs == 1) {
    dcmpMethod = mcRPL::SNGL_PRC;
  }
  else if(nRowSubspcs > 1 && nColSubspcs == 1) {
    dcmpMethod = mcRPL::SMPL_ROW;
  }
  else if(nRowSubspcs == 1 && nColSubspcs > 1) {
    dcmpMethod = mcRPL::SMPL_COL;
  }
  else if(nRowSubspcs > 1 && nColSubspcs > 1) {
    dcmpMethod = mcRPL::SMPL_BLK;
  }
  else {
    cerr << __FILE__ << " " << __FUNCTION__ \
...
copyDcmp(const Layer *pFromLyr) {
  ...
  for(int iSubInfo = 0; iSubInfo < pFromLyr->nSubCellspaceInfos(); iSubInfo++) {
    _lSubInfos.push_back(mcRPL::SubCellspaceInfo(*(pFromLyr->subCellspaceInfo_lclID(iSubInfo))));
  }
  return true;
}
...
initCellspaceInfo() {
  delCellspaceInfo();
  delCellspace();
  _pCellspcInfo = new mcRPL::CellspaceInfo();
}

void mcRPL::Layer::
initCellspaceInfo(const mcRPL::SpaceDims &dims,
  ...
  delCellspaceInfo();
  delCellspace();
  _pCellspcInfo = new mcRPL::CellspaceInfo(dims, aTypeName, typeSize, pGeoinfo, tileSize);
}

void mcRPL::Layer::
initCellspaceInfo(const mcRPL::CellspaceInfo &cellspcInfo) {
  delCellspaceInfo();
  delCellspace();
  _pCellspcInfo = new mcRPL::CellspaceInfo(cellspcInfo);
}
...
mcRPL::CellspaceInfo* mcRPL::Layer::
cellspaceInfo() {
  return _pCellspcInfo;
}

const mcRPL::CellspaceInfo* mcRPL::Layer::
cellspaceInfo() const {
  return _pCellspcInfo;
}
...
createCellspace(double *pInitVal) {
  ...
  _pCellspc = new mcRPL::Cellspace(_pCellspcInfo, true);
...
/*---------------------PGTIOL----------------------------*/
bool mcRPL::Layer::loadCellspaceByPGTIOL() {
...
bool mcRPL::Layer::writeCellspaceByPGTIOL() {
  if(_pCellspcInfo == NULL ||
	 _pCellspc == NULL) {
	cerr << __FILE__ << " " << __FUNCTION__ \
		 << " Error: NULL CellspaceInfo or NULL Cellspace, unable to write to a GDAL dataset" \
...
mcRPL::Cellspace* mcRPL::Layer::
cellspace() {
  return _pCellspc;
}

const mcRPL::Cellspace* mcRPL::Layer::
cellspace() const {
  return _pCellspc;
}
...
const list<mcRPL::SubCellspaceInfo>* mcRPL::Layer::
...
delSubCellspaceInfo_glbID(int glbID) {
  bool found = false;
  list<mcRPL::SubCellspaceInfo>::iterator itrSubInfo = _lSubInfos.begin();
...
mcRPL::SubCellspaceInfo* mcRPL::Layer::
subCellspaceInfo_glbID(int glbID,
                       bool warning) {
  mcRPL::SubCellspaceInfo *pSubInfo = NULL;
  list<mcRPL::SubCellspaceInfo>::iterator itrSubInfo = _lSubInfos.begin();
  while(itrSubInfo != _lSubInfos.end()) {
    if(glbID == itrSubInfo->id()) {
      pSubInfo = &(*itrSubInfo);
...
const mcRPL::SubCellspaceInfo* mcRPL::Layer::
subCellspaceInfo_glbID(int glbID,
                       bool warning) const {
  const mcRPL::SubCellspaceInfo *pSubInfo = NULL;
  list<mcRPL::SubCellspaceInfo>::const_iterator itrSubInfo = _lSubInfos.begin();
  while(itrSubInfo != _lSubInfos.end()) {
    if(glbID == itrSubInfo->id()) {
      pSubInfo = &(*itrSubInfo);
      break;
...
const mcRPL::IntVect mcRPL::Layer::
allSubCellspaceIDs() const {
  mcRPL::IntVect vSubspcIDs;
  list<mcRPL::SubCellspaceInfo>::const_iterator itrSubInfo = _lSubInfos.begin();
  while(itrSubInfo != _lSubInfos.end()) {
...
mcRPL::SubCellspace* mcRPL::Layer::
addSubCellspace(int glbID,
                double *pInitVal) {
  mcRPL::SubCellspace *pSubspc = subCellspace_glbID(glbID);
  if(pSubspc != NULL) {
    cerr << __FILE__ << " " << __FUNCTION__ \
    ...
    mcRPL::SubCellspaceInfo* pSubspcInfo = subCellspaceInfo_glbID(glbID);
    if(pSubspcInfo == NULL) {
      cerr << __FILE__ << " " << __FUNCTION__ \
      ...
      _lCellspcInfos.push_back(mcRPL::CellspaceInfo(pSubspcInfo->dims(),dataType(),dataSize()));
      mcRPL::CellspaceInfo *pCellspcInfo_sub = &(_lCellspcInfos.back());

      double noDataVal = _pCellspcInfo->getNoDataValAs<double>();
      pCellspcInfo_sub->setNoDataValAs<double>(noDataVal);

      if(_pCellspcInfo->isGeoreferenced(false)) {
        mcRPL::CellspaceGeoinfo subspcGeoinfo = _pCellspcInfo->georeference()->subSpaceGeoinfo(*pSubspcInfo);
        pCellspcInfo_sub->georeference(subspcGeoinfo);
      }

      pCellspcInfo_sub->setTileSize(tileSize());

      _lSubCellspcs.push_back(mcRPL::SubCellspace(pCellspcInfo_sub, pSubspcInfo));
...
delSubCellspace_glbID(int glbID) {
  bool found = false;
  list<mcRPL::SubCellspace>::iterator itrSubspc = _lSubCellspcs.begin();
  list<mcRPL::CellspaceInfo>::iterator itrCellspcInfo = _lCellspcInfos.begin();
...
subCellspace_glbID(int glbID,
                   bool warning) {
  mcRPL::SubCellspace *pSubspc = NULL;
  list<mcRPL::SubCellspace>::iterator itrSubspc = _lSubCellspcs.begin();
  while(itrSubspc != _lSubCellspcs.end()) {
    if(glbID == itrSubspc->subInfo()->id()) {
      pSubspc = &(*itrSubspc);
...
const mcRPL::SubCellspace* mcRPL::Layer::
subCellspace_glbID(int glbID,
                   bool warning) const {
  const mcRPL::SubCellspace *pSubspc = NULL;
  list<mcRPL::SubCellspace>::const_iterator itrSubspc = _lSubCellspcs.begin();
  while(itrSubspc != _lSubCellspcs.end()) {
    if(glbID == itrSubspc->subInfo()->id()) {
      pSubspc = &(*itrSubspc);
      break;
...
allUpdatedIdxs(vector<long> &vUpdtGlbIdxs) const {
  ...
  if(_lSubCellspcs.size() > 0) {
    list<mcRPL::SubCellspace>::const_iterator itrSubspc = _lSubCellspcs.begin();
    while(itrSubspc != _lSubCellspcs.end()) {
      const mcRPL::LongVect &vLclIdxs = itrSubspc->getUpdatedIdxs();
...
{
	mcRPL::SubCellspace *pSubspc = subCellspace_glbID(glbID, false);
  if(pSubspc == NULL) {
...
writeSubCellspaceByGDAL(int glbID) {
  mcRPL::SubCellspace *pSubspc = subCellspace_glbID(glbID, true);
  if(pSubspc == NULL) {
    return false;
  }

  const mcRPL::CellCoord gdalOff = pSubspc->subInfo()->MBR().nwCorner();
  const mcRPL::CoordBR workBR = pSubspc->subInfo()->workBR();
