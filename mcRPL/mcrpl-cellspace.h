#ifndef MCRPL_CELLSPACE_H
#define MCRPL_CELLSPACE_H
...
namespace mcRPL {
  class Cellspace {
    public:
      ...
      bool init(mcRPL::CellspaceInfo *pInfo,
                bool ifInitMem = true);
      ...
      /* Operators */
      mcRPL::Cellspace& operator=(const mcRPL::Cellspace& rhs);
	  template<class InputType>
	 bool brupdateCellAs(const mcRPL::CoordBR &br,void *output);
	 template<class InputType>
	 ...
	 bool createrandForCells(long ncols,long nrows);
	  ...
	  void deleteGPUData();
      /* Information */
      const mcRPL::CellspaceInfo* info() const;
      mcRPL::CellspaceInfo* info();
      void info(mcRPL::CellspaceInfo *pInfo);
      ...
      template<class RtrType>
      RtrType& at(const mcRPL::CellCoord &coord,
                  bool warning = true);
      template<class RtrType>
      const RtrType& at(const mcRPL::CellCoord &coord,
                        bool warning = true) const;
      ...
      template<class RtrType>
      RtrType atAs(const mcRPL::CellCoord &coord,
                   bool warning = true) const;
      template<class RtrType>
      ...
      bool values(vector<RtrType> *pvVals,
                  const mcRPL::CoordBR *pRectangle = NULL,
                  bool warning = true) const;
       ...
       */
      template<class ElemType>
      bool find(mcRPL::LongVect *pvFoundIdxs,
                const ElemType *pVal,
                const mcRPL::CoordBR *pRectangle = NULL,
                bool warning = true) const;
      ...
      template<class RtrType>
      RtrType nbrhdTotalVal(const mcRPL::Neighborhood &nbrhd,
                            const mcRPL::CellCoord &ctrCoord,
                            bool includeCtr = true,
                            bool warning = true) const;
      template<class RtrType>
      ...
      template<class RtrType>
      RtrType nbrhdTotalValAs(const mcRPL::Neighborhood &nbrhd,
                              const mcRPL::CellCoord &ctrCoord,
                              bool includeCtr = true,
                              bool warning = true) const;
      template<class RtrType>
      ...
      const mcRPL::LongVect& getUpdatedIdxs() const;
      void clearUpdatedIdxs();
      bool brupdateCell(const mcRPL::CoordBR &br,void *output);
      template<class ElemType>
      bool updateCell(const mcRPL::CellCoord &coord, const ElemType &val,
                      bool warning = true);
      template<class ElemType>
      ...
      template<class InputType>
      bool updateCellAs(const mcRPL::CellCoord &coord, const InputType &val,
                        bool warning = true);
      template<class InputType>
      ...
      template<class ElemType>
      bool updateCells(const vector< pair<mcRPL::CellCoord, ElemType> > &vCells,
                       bool warning = true);
      template<class ElemType>
      ...
      template<class InputType>
      bool updateCellsAs(const vector< pair<mcRPL::CellCoord, InputType> > &vCells,
                         bool warning = true);
      template<class InputType>
      ...
      */
      ...
      bool readGDALData(GDALDataset *pDataset,
                        int iBand = 1,
                        const mcRPL::CellCoord *pGDALOff = NULL,
                        bool warning = true);
	  bool creatRandData( const mcRPL::CellCoord *pGDALOff = NULL,
                        bool warning = true);
      bool writeGDALData(GDALDataset *pDataset,
                         int iBand = 1,
                         const mcRPL::CellCoord *pGDALOff = NULL,
                         const mcRPL::CoordBR *pWorkBR = NULL,
                         bool warning = true);
	  /*pgtiol--------------*/
    ...
    protected:
      mcRPL::CellspaceInfo *_pInfo;
      void *_pData;
	  void *_dpData;
      bool _trackUpdt;
      mcRPL::LongVect _vUpdtIdxs;
...
template <class RtrType> 
RtrType& mcRPL::Cellspace::
at(const mcRPL::CellCoord& coord,
   bool warning) {
  RtrType* pElem = NULL;
  if(!isEmpty(warning) &&
...
template<class RtrType>
RtrType mcRPL::Cellspace::
atAs(const mcRPL::CellCoord &coord,
     bool warning) const {
...
values(vector<RtrType> *pvVals,
       const mcRPL::CoordBR *pRectangle,
       bool warning) const {
  bool done = true;
  if(pvVals == NULL) {
    if(warning) {
...
valuesAs(vector<RtrType> *pvVals,
  ...
  if(pRectangle == NULL) {
    ...
    for(long iElem = 0; iElem < mySize; iElem++) {
      ...
      if(fabs(val - (RtrType)mcRPL::ERROR_VAL) < mcRPL::EPSINON) {
        return false;
      }
...
template<class ElemType>
bool mcRPL::Cellspace::
find(mcRPL::LongVect *pvFoundIdxs,
     const vector<ElemType> *pvVals,
     const mcRPL::CoordBR *pRectangle,
     bool warning) const {
  bool found = false;
    ...
    if(_pInfo->isDataType<ElemType>(warning) && !isEmpty(warning)) {
      if(pRectangle == NULL) {
        ...
        while(pElem != (const ElemType*)_pData + mySize) {
          ...
          if(pElem != (const ElemType*)_pData + mySize) {
            if(excldNoData &&
               fabs(*pElem - _pInfo->getNoDataValAs<ElemType>()) < mcRPL::EPSINON) {
              continue;
            }
            else {
              pvFoundIdxs->push_back(pElem - (const ElemType*)_pData);
...
nbrhdVals(vector<RtrType>& vVals,
  ...
  if(_pInfo->isDataType<RtrType>(warning) &&
    ...
    for(int iNbr = 0; iNbr < nbrhd.size(); iNbr++) {
      ...
      mcRPL::CellCoord nbrCoord(ctrCoord+nbrhd[iNbr]);
      if(nbrCoord.valid(_pInfo->dims())) {
        ...
        case mcRPL::NODATA_VIRTUAL_EDGES:
          vVals.push_back(_pInfo->getNoDataVal<RtrType>());
          break;
        case mcRPL::CUSTOM_VIRTUAL_EDGES:
...
nbrhdVals(vector<RtrType>& vVals,
          ...
          bool includeCtr,
          bool warning) const {
  return nbrhdVals<RtrType>(vVals, nbrhd, mcRPL::CellCoord(ctrRow, ctrCol), includeCtr, warning);
}
...
nbrhdValsAs(vector<RtrType> &vVals,
            ...
            bool includeCtr,
            bool warning) const {
  return nbrhdValsAs<RtrType>(vVals, nbrhd, mcRPL::CellCoord(ctrRow, ctrCol), includeCtr, warning);
}
...
nbrhdFreqs(map<RtrType, int> &mFreqs,
           ...
           bool includeCtr,
           bool warning) const {
  return nbrhdFreqs<RtrType>(mFreqs, nbrhd, mcRPL::CellCoord(ctrRow, ctrCol), includeCtr, warning);
}
...
wnbrhdFreqs(map<RtrType, double> &mFreqs,
            ...
            bool includeCtr,
            bool warning) const {
  return wnbrhdFreqs<RtrType>(mFreqs, nbrhd, mcRPL::CellCoord(ctrRow, ctrCol), includeCtr, warning);
}
...
template<class RtrType>
RtrType mcRPL::Cellspace::
nbrhdTotalVal(const mcRPL::Neighborhood& nbrhd,
              const mcRPL::CellCoord& ctrCoord,
              bool includeCtr,
              bool warning) const {
  ...
  if(_pInfo->isDataType<RtrType>(warning) &&
    ...
    for(int iNbr = 0; iNbr < nbrhd.size(); iNbr++) {
      ...
      const mcRPL::WeightedCellCoord& wCoord = nbrhd[iNbr];
      mcRPL::CellCoord nbrCoord(ctrCoord+wCoord);
      if(nbrCoord.valid(_pInfo->dims())) {
        const RtrType& val = *((const RtrType*)_pData+nbrCoord.iRow()*(_pInfo->nCols())+nbrCoord.iCol());
        if(fabs(val - _pInfo->getNoDataValAs<RtrType>()) >= mcRPL::EPSINON) {
          total += val * wCoord.weight();
        }
      }
      else if(nbrhd.edgeOption() == mcRPL::CUSTOM_VIRTUAL_EDGES) {
        if(nbrhd.virtualEdgeVal() != NULL) {
...
nbrhdTotalVal(const mcRPL::Neighborhood& nbrhd,
                ...
                bool includeCtr,
                bool warning) const {
  return nbrhdTotalVal<RtrType>(nbrhd, mcRPL::CellCoord(ctrRow, ctrCol),
                                includeCtr, warning);
}
...
template<class RtrType>
RtrType mcRPL::Cellspace::
nbrhdTotalValAs(const mcRPL::Neighborhood& nbrhd,
                const mcRPL::CellCoord& ctrCoord,
                bool includeCtr,
                bool warning) const {
  RtrType total = RtrType(0.0);
...
nbrhdTotalValAs(const mcRPL::Neighborhood& nbrhd,
                  ...
                  bool includeCtr,
                  bool warning) const {
  return nbrhdTotalValAs<RtrType>(nbrhd, mcRPL::CellCoord(ctrRow, ctrCol),
                                  includeCtr, warning);
}
...
template<class ElemType>
bool mcRPL::Cellspace::
updateCell(const mcRPL::CellCoord &coord, const ElemType &val,
           bool warning) {
  bool done = false;
...
updateCell(long iRow, long iCol, const ElemType &val,
           bool warning) {
  return updateCell(mcRPL::CellCoord(iRow, iCol), val, warning);
}
...
template<class InputType>
bool mcRPL::Cellspace::
updateCellAs(const mcRPL::CellCoord &coord, const InputType &val,
             bool warning) {
  bool done = false;
  if(_pInfo->isDataType<bool>()) {
...
updateCellAs(long iRow, long iCol, const InputType &val,
             bool warning) {
  return updateCellAs<InputType>(mcRPL::CellCoord(iRow, iCol), val, warning);
}
...
template<class ElemType>
bool mcRPL::Cellspace::
updateCells(const vector< pair<mcRPL::CellCoord, ElemType> > &vCells,
            bool warning) {
  bool done = true;
...
template<class InputType>
bool mcRPL::Cellspace::
updateCellsAs(const vector< pair<mcRPL::CellCoord, InputType> > &vCells,
              bool warning) {
  bool done = true;
  for(size_t iCell = 0; iCell < vCells.size(); iCell++) {
...
template<class InputType>
bool mcRPL::Cellspace::
brupdateCellAs(const mcRPL::CoordBR &br,void *output)
{
	 bool done = false;
	 for(long iRow = br.minIRow(); iRow <= br.maxIRow(); iRow++) 
	  {
		  for(long iCol = br.minICol(); iCol <= br.maxICol(); iCol++) 
		  {
			  ...
			  return done;
				 ...
				 done=updateCellAs<InputType>(mcRPL::CellCoord(iRow, iCol),((InputType*)output)[iRow*_pInfo->nCols()+iCol], true);                         //”–Œ Ã‚
				 if(!done)
					 return done;
		  }
	  }
